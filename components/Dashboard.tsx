...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const exportToCSV = () => {
    ...
    // Excel (özellikle Türkçe sistemlerde) sütunları ayırmak için noktalı virgül (;) bekler.
    const headers = "İngilizce Kelime;Türkçe Karşılığı;Örnek Cümle;Türkçe Cümle\n";
    
    const rows = words.map(w => {
        // Excel CSV formatında hücre içindeki tırnak işaretleri çift tırnak ("") ile kaçış yapılmalıdır.
    ...
    // BOM (\uFEFF) karakteri Türkçe karakterlerin (ğ, ü, ş, ı, ö, ç) bozuk görünmemesi için gereklidir.
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-blue-500/30 flex justify-center">
      ...
      <div className="w-full max-w-6xl bg-black min-h-screen border-x border-white/5 shadow-[0_0_150px_rgba(255,255,255,0.02)] relative">
        ...
        <div className="p-8 flex justify-between items-center">
          <div className="flex items-center gap-5 group select-none cursor-default">
            ...
            <div>
              <h1 className="text-3xl font-black text-white tracking-tighter leading-none flex items-center">
                Lingua<span className="text-blue-500">Card</span>
              </h1>
              <div className="flex items-center gap-2 mt-1 pl-0.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div>
                  <p className="text-slate-500 text-[11px] font-extrabold tracking-[0.25em] uppercase">
                    Kelime Öğren
                  </p>
              </div>
            </div>
          ...
          <div className="flex gap-3">
              {/* Logout Red Button */}
              <button onClick={onLogout} className="p-4 bg-red-500/10 text-red-500 rounded-2xl border border-red-500/20 hover:bg-red-500 hover:text-white transition-all active:scale-95 shadow-lg shadow-red-500/10">
                  <LogOut size={20} />
              </button>
          </div>
        </div>

        {/* Hero Banner - More Distinct Background */}
        <div className="px-8 my-6">
          <div className="bg-gradient-to-r from-indigo-900 via-blue-900 to-slate-900 p-10 rounded-[48px] border border-blue-500/30 relative overflow-hidden group shadow-2xl shadow-blue-900/20">
              <div className="absolute top-0 right-0 w-80 h-80 bg-white/5 rounded-full blur-[80px] group-hover:bg-white/10 transition-all"></div>
        ...
        {/* Modes Grid - Distinct Card Colors */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-8 mb-16">
          <button onClick={() => onModeSelect(AppMode.FLASHCARDS)} className="bg-zinc-900 p-8 rounded-[40px] border-b-4 border-yellow-500 hover:border-yellow-400 transition-all text-left group hover:bg-zinc-800">
            <div className="bg-yellow-500/10 w-16 h-16 rounded-3xl flex items-center justify-center mb-8 text-yellow-500 group-hover:scale-110 group-hover:bg-yellow-500/20 transition-all"><BookOpen size={32}/></div>
          ...
          <button onClick={() => onModeSelect(AppMode.QUIZ)} className="bg-zinc-900 p-8 rounded-[40px] border-b-4 border-emerald-500 hover:border-emerald-400 transition-all text-left group hover:bg-zinc-800">
            <div className="bg-emerald-500/10 w-16 h-16 rounded-3xl flex items-center justify-center mb-8 text-emerald-500 group-hover:scale-110 group-hover:bg-emerald-500/20 transition-all"><Puzzle size={32}/></div>
          ...
          <button onClick={() => onModeSelect(AppMode.SENTENCES)} className="bg-zinc-900 p-8 rounded-[40px] border-b-4 border-purple-500 hover:border-purple-400 transition-all text-left group hover:bg-zinc-800">
            <div className="bg-purple-500/10 w-16 h-16 rounded-3xl flex items-center justify-center mb-8 text-purple-500 group-hover:scale-110 group-hover:bg-purple-500/20 transition-all"><Sparkles size={32}/></div>
        ...
        <div className="px-8 pb-20">
          <div className="flex justify-between items-center mb-8 px-2">
              ...
              <div className="flex items-center gap-4">
                  ...
                  <button onClick={exportToCSV} className="flex items-center gap-2 px-4 py-2 bg-orange-500/10 text-orange-500 rounded-xl border border-orange-500/20 hover:bg-orange-500 hover:text-white transition-all active:scale-95">
                      <Download size={18} />
